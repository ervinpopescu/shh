...
public enum SendImageValidator {
    ...
    public static func validate(_ data: Data, limits: SendImageLimits = SendImageValidator.default)
        throws -> ValidatedSendImage
    {
        ...
        guard data.count <= limits.maximumBytes else { throw SendImageError.imageTooLarge }
            ...
            return ValidatedSendImage(
                data: data, fileExtension: fileExtension, uniformTypeIdentifier: identifier)
        #else
            throw SendImageError.unsupportedImage
        #endif
    }
...
public enum SendImageDestination {
    ...
    /// Validates an optional host destination override path. Rejects empty strings,
    /// non-absolute paths, root paths, and any ASCII control characters or DEL (such as
    /// carriage returns and newlines) to prevent line injection into the terminal prompt.
...
public enum SendImageNaming {
    ...
    /// returns bytes only - callers must not append a line ending. Rejects empty
    /// strings and control characters or DEL (including carriage returns and newlines)
    /// to prevent prompt injection or premature execution.
