        ...
        if capabilities.kind == .tmux {
            if let sessionID = tmuxSessionID {
                if capabilities.contains(.previousWindow) {
                    nodes.append(
                        DialNode(
                            id: "mux.tmux.previous-window", title: "Previous Window",
                            systemImage: "chevron.left.2",
                            action: .multiplexerControl(.tmux(.previousWindow(sessionID)))))
                }
                if capabilities.contains(.nextWindow) {
                    nodes.append(
                        DialNode(
                            id: "mux.tmux.next-window", title: "Next Window",
                            systemImage: "chevron.right.2",
                            action: .multiplexerControl(.tmux(.nextWindow(sessionID)))))
                }
                ...
                if capabilities.contains(.directionalFocus) {
                    for direction in TmuxPaneDirection.allCases {
                        nodes.append(
                            DialNode(
                                id: "mux.tmux.focus.\(direction.rawValue)",
                                title: "Focus \(direction.rawValue.capitalized)",
                                systemImage: "arrow.\(direction.rawValue)",
                                action: .multiplexerControl(
                                    .tmux(.focusPane(target, direction: direction)))))
                    }
                }
                if capabilities.contains(.horizontalSplit) {
                    nodes.append(
                        DialNode(
                            id: "mux.tmux.split.horizontal", title: "Split Horizontal",
                ...
                if capabilities.contains(.verticalSplit) {
                    nodes.append(
                        DialNode(
                            id: "mux.tmux.split.vertical", title: "Split Vertical",
                ...
                if capabilities.contains(.zoom) {
                    nodes.append(
                        DialNode(
                            id: "mux.tmux.zoom", title: "Toggle Zoom",
                ...
                if capabilities.contains(.copyMode) {
                    nodes.append(
                        DialNode(
                            id: "mux.tmux.copy-mode", title: "Copy Mode",
                ...
                if capabilities.contains(.closePane) {
                    nodes.append(
                        DialNode(
                            id: "mux.tmux.close-pane", title: "Close Pane",
                            systemImage: "rectangle.badge.xmark",
                            action: .multiplexerControl(.tmux(.closePane(target))),
                            availability: .reviewRequired))
                }
            }
        }
        return nodes
