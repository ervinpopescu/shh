...
public struct TmuxClientTTY: Hashable, Sendable, Codable, CustomStringConvertible {
    ...
    public init(_ value: String) throws {
        guard value.hasPrefix("/"), value.count <= 512,
              value.allSatisfy({ $0.isASCII && ($0.isLetter || $0.isNumber || $0 == "/" || $0 == "_" || $0 == "-" || $0 == ".") }) else {
            throw MultiplexerControlError.invalidIdentifier(value)
        }
        self.value = value
    }
...
public struct TmuxControlTarget: Hashable, Sendable, Codable {
    ...
    public init(sessionID: TmuxSessionID, window: TmuxWindowTarget? = nil,
...
public enum MultiplexerCapability: String, CaseIterable, Hashable, Sendable, Codable {
    case attachSession, createSession, previousWindow, nextWindow, directionalFocus, horizontalSplit, verticalSplit
    case zoom, rename, sessionPicker, windowPicker, copyMode, closePane, detachClient
    case workspaceList, workspaceCreate, tabCreate, paneList, paneSplit, paneRun, paneRead, waitAgentStatus, status
}
...
public struct MultiplexerCapabilities: Hashable, Sendable, Codable {
    ...
    public func contains(_ capability: MultiplexerCapability) -> Bool { supported.contains(capability) }

    public static let tmux = MultiplexerCapabilities(kind: .tmux, supported: [
    ...
    public static let herdr = MultiplexerCapabilities(kind: .herdr, supported: [
...
public protocol MultiplexerControl: Sendable {
    ...
    func execute(_ action: MultiplexerControlAction, using executor: any SSHCommandExecuting) async throws -> SSHCommandResult
}

private func validatedHerdrIdentifier(_ value: String) throws -> String {
    guard !value.isEmpty, value.count <= 256,
          !value.unicodeScalars.contains(where: { $0.value < 0x20 || $0.properties.isWhitespace }) else {
        throw MultiplexerControlError.invalidIdentifier(value)
    }
    return value
}
...
private func executablePrefix(_ executable: String) throws -> String {
    guard executable == "tmux" || executable == "herdr" ||
            (executable.hasPrefix("/") && executable.count <= 512 &&
             !executable.unicodeScalars.contains(where: { $0.value < 0x20 || $0.properties.isWhitespace })) else {
        throw MultiplexerControlError.invalidIdentifier(executable)
    }
    return executable == "tmux" || executable == "herdr" ? executable : ShellQuoting.quote(executable)
}
...
public struct TmuxControl: MultiplexerControl {
    ...
    public func command(for action: MultiplexerControlAction) throws -> String {
        guard case .tmux(let action) = action else { throw MultiplexerControlError.unsupportedAction }
        let binary = executable
        switch action {
        case .attachSession(let session): return "env -u TMUX \(binary) attach-session -d -t \(session.shellArgument)"
        case .createSession(let name): return "\(binary) new-session -A -D -s \(name.shellArgument)"
        case .previousWindow(let session): return "\(binary) previous-window -t \(session.shellArgument)"
        case .nextWindow(let session): return "\(binary) next-window -t \(session.shellArgument)"
        case .focusPane(let target, let direction): return "\(binary) select-pane -t \(target.shellArgument) -\(direction.flag)"
        case .split(let target, let vertical): return "\(binary) split-window -\(vertical ? "v" : "h") -t \(target.shellArgument)"
        case .toggleZoom(let target): return "\(binary) resize-pane -Z -t \(target.shellArgument)"
        case .renameWindow(let target, let name): return "\(binary) rename-window -t \(target.shellArgument) \(name.shellArgument)"
        case .sessionPicker(let session): return "\(binary) choose-tree -s -w -t \(session.shellArgument)"
    ...
    public func execute(_ action: MultiplexerControlAction, using executor: any SSHCommandExecuting) async throws -> SSHCommandResult {
        if case .tmux(.detachClient(let session, let expectedTTY)) = action, expectedTTY == nil {
            ...
            return try await executor.executeCommand(command(for: .tmux(.detachClient(session: session, expectedTTY: identity.tty))))
        }
...
public struct HerdrControl: MultiplexerControl {
    ...
    public func command(for action: MultiplexerControlAction) throws -> String {
        guard case .herdr(let action) = action else { throw MultiplexerControlError.unsupportedAction }
        let binary = executable
        switch action {
        case .workspaceList: return "\(binary) workspace list --format 'json'"
        case .workspaceCreate(let cwd, let label): return "\(binary) workspace create --cwd \(ShellQuoting.quote(cwd)) --label \(ShellQuoting.quote(label))"
        case .tabCreate(let label): return "\(binary) tab create --label \(ShellQuoting.quote(label))"
        case .paneList(let workspaceID):
            if let workspaceID { return "\(binary) pane list \(ShellQuoting.quote(try validatedHerdrIdentifier(workspaceID))) --format 'json'" }
            return "\(binary) pane list --format 'json'"
        case .paneSplit(let paneID, let direction):
            guard ["left", "right", "up", "down"].contains(direction.lowercased()) else { throw MultiplexerControlError.unsupportedAction }
            return "\(binary) pane split \(ShellQuoting.quote(try validatedHerdrIdentifier(paneID))) --direction \(ShellQuoting.quote(direction.lowercased()))"
        case .paneRun(let paneID, let command):
            return "\(binary) pane run \(ShellQuoting.quote(try validatedHerdrIdentifier(paneID))) \(ShellQuoting.quote(command))"
        case .paneRead(let paneID, let source):
            return "\(binary) pane read \(ShellQuoting.quote(try validatedHerdrIdentifier(paneID))) --source \(ShellQuoting.quote(try validatedHerdrIdentifier(source)))"
        case .waitAgentStatus(let paneID, let status):
            var result = "\(binary) wait agent-status"
            if let paneID { result += " \(ShellQuoting.quote(try validatedHerdrIdentifier(paneID)))" }
            if let status { result += " --status \(ShellQuoting.quote(try validatedHerdrIdentifier(status)))" }
            return result
...
public enum TmuxClientResolver {
    ...
    public static func parse(_ output: String) throws -> [TmuxClientIdentity] {
        try output.split(whereSeparator: \.isNewline).map { line in
            ...
            guard fields.count == 2,
                  ...
                  let session = try? TmuxSessionID(String(fields[1])) else {
    ...
    public static func resolve(sessionID: TmuxSessionID, expectedTTY: TmuxClientTTY? = nil,
                               using executor: any SSHCommandExecuting) async throws -> TmuxClientIdentity {
        ...
        if let expectedTTY {
            ...
            guard matches.count == 1, let match = matches.first else {
                throw matches.isEmpty ? MultiplexerControlError.clientNotFound : MultiplexerControlError.ambiguousClient
            }
            return match
        }
        guard clients.count == 1, let only = clients.first else {
            throw clients.isEmpty ? MultiplexerControlError.clientNotFound : MultiplexerControlError.ambiguousClient
        }
        return only
    }
}
