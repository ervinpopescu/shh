...
public struct TerminalScrollIntentReducer: Sendable {
    ...
    public init(
        thresholdRows: Double = 1.0,
        maximumVelocity: Double = 4_000,
        maximumIntentsPerUpdate: Int = 4
    ) {
    ...
    public mutating func reduce(
        ...
        case .began:
            state = State()
            state.active = true
            if context.surface == .alternate
                && !context.mouseReporting
                && context.copyModeFallbackAvailable
            {
        ...
        case .changed:
            ...
            guard context.surface == .alternate || context.mouseReporting else {
                return [.native]
            }

            let velocity = min(
                max(gesture.velocityY.isFinite ? gesture.velocityY : 0, -maximumVelocity),
                maximumVelocity
            )
