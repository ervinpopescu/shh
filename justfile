...
format:
    ...
    swift-format format --in-place --recursive App FileProviderExtension Shared Sources Tests

# Check Swift formatting without changing files.
format-check:
    #!/usr/bin/env bash
    command -v swift-format >/dev/null 2>&1 || { echo "Error: swift-format is required (install it with brew install swift-format)" >&2; exit 1; }
    swift-format lint --recursive App FileProviderExtension Shared Sources Tests
