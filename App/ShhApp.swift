...
struct SessionView: View {
    ...
    private var isPinnedLiteralPresented: Binding<Bool> {
        Binding(get: { pendingPinnedLiteral != nil }, set: { if !$0 { pendingPinnedLiteral = nil } })
    }
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            ZStack(alignment: container.commandDialPreferences.placement == .leading ? .bottomLeading : .bottomTrailing) {
                ...
                if isCommandDialOpen {
                    CommandDialSurface(
                        ...
                        haptics: container.commandDialPreferences.hapticsEnabled ? SystemDialHaptics() : NoopDialHaptics()
                    )
                }
            }
            .overlay(alignment: container.commandDialPreferences.placement == .leading ? .bottomLeading : .bottomTrailing) {
        ...
        .confirmationDialog("Review pinned text", isPresented: isPinnedLiteralPresented, presenting: pendingPinnedLiteral) { literal in
        ...
        .confirmationDialog("Confirm multiplexer control", isPresented: isDialControlPresented, presenting: pendingDialControl) { control in
    ...
    private var commandDialModel: CommandDialModel {
        let descriptors = dialSnippets.map { snippet in
            ...
            return DialSnippetDescriptor(id: snippet.id, name: snippet.name,
    ...
    private var commandDialMultiplexerNodes: [DialNode] {
        ...
        var nodes: [DialNode] = []
        let tmuxRawID = container.activeTmuxSessionID.flatMap { raw in
            (try? TmuxSessionID(raw))?.value
                ?? container.tmuxSessions.first(where: { $0.name == raw || $0.sessionID == raw })?.sessionID
        } ?? container.tmuxSessions.first?.sessionID
        if let tmuxRawID, let sessionID = try? TmuxSessionID(tmuxRawID), container.tmuxAvailability.isAvailable {
        ...
        if container.herdrAvailability.isAvailable {
            nodes += CommandDialMultiplexerMenu.nodes(
                tmuxSessionID: nil, herdrWorkspaceID: container.activeHerdrWorkspaceID, capabilities: .herdr
            )
        }
        return nodes
    }
    ...
    private func handleDialAction(_ action: DialActionIdentifier) {
        ...
        case .keyboard:
            ...
            container.terminalController.recoverFirstResponder()
        case .multiplexer:
            if commandDialModel.roots.first(where: { $0.action == .multiplexer })?.children.isEmpty == false {
                commandDialNavigation.clearSelection()
            } else {
                dismissCommandDial()
                showMultiplexer = true
    ...
    private func insertPinnedLiteral(_ literal: String, approved: Bool = false) {
        guard CommandDialModel.isInsertOnlyTerminalText(literal), policy.classify(literal) != .blocked else { return }
...
struct SettingsView: View {
    ...
    private func addDialLiteral() {
        ...
        preferences.pinnedLiterals = CommandDialModel.validPinnedLiterals(preferences.pinnedLiterals + [value])
        ...
        Form {
            ...
            Section("Command Dial") {
                Picker("Placement", selection: Binding(
                    ...
                    set: { value in var preferences = container.commandDialPreferences; preferences.placement = value; container.setCommandDialPreferences(preferences) }
                )) {
                    ForEach(CommandDialPlacement.allCases) { placement in Text(placement.title).tag(placement) }
                }
                .accessibilityIdentifier("settings-command-dial-placement")

                Picker("Size", selection: Binding(
                    get: { container.commandDialPreferences.size },
                    set: { value in var preferences = container.commandDialPreferences; preferences.size = value; container.setCommandDialPreferences(preferences) }
                )) {
                ...
                Toggle("Haptic feedback", isOn: Binding(
                    ...
                    set: { value in var preferences = container.commandDialPreferences; preferences.hapticsEnabled = value; container.setCommandDialPreferences(preferences) }
                ))
                ...
                HStack {
                    ...
                    Button("Add", action: addDialLiteral)
                        .disabled(newDialLiteral.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty)
                }
                if !container.commandDialPreferences.pinnedLiterals.isEmpty {
                    ForEach(container.commandDialPreferences.pinnedLiterals, id: \.self) { literal in
                        HStack {
