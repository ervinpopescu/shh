...
- SFTP image insertion performs local ImageIO content and dimension
  ...
  SFTP channel to user-owned storage, and inserts the POSIX-quoted
  remote path into the terminal PTY without a trailing newline,
  preventing automatic or unintentional command execution.
