...
final class TerminalScrollIntentTests: XCTestCase {
    ...
    func testPrimaryMultiplexerWithMouseReportingStaysNative() {
        var reducer = TerminalScrollIntentReducer()
        var state = TerminalScrollIntentReducer.State()
        let context = TerminalScrollContext(
            surface: .primary,
            mouseReporting: true,
            multiplexerActive: true
        )
    ...
    func testTerminalControllerCopyModeFallbackAvailabilityGating() {
        ...
        XCTAssertFalse(controller.isMultiplexerActive)
...
extension TerminalScrollIntentTests {
    ...
    func testHostViewMouseModeTranslatesSwipesToWheelEvents() {
        ...
        ptyOutput.removeAll()
        // A physical downward swipe reports wheel-down.
        hostView.processScrollGesture(
            phase: .changed,
            translationY: 50,
            location: CGPoint(x: 100, y: 100)
    ...
    @MainActor
    func testHostViewPrimaryTmuxMouseReportingUsesNativeScrollback() {
        let controller = ShhTerminalController()
        let hostView = ShhInternalTerminalHostView(
            frame: CGRect(x: 0, y: 0, width: 393, height: 852),
            options: .default,
        ...
        XCTAssertTrue(controller.isMultiplexerActive)
        XCTAssertFalse(
            hostView.gestureRecognizerShouldBegin(hostView.scrollGesture),
            "Primary tmux scrolling must remain owned by SwiftTerm's native scrollback"
        )
        XCTAssertFalse(hostView.allowMouseReporting, "Native tmux scrollback must suppress SwiftTerm mouse reports")
        ...
        XCTAssertTrue(ptyOutput.isEmpty, "Native primary tmux scrolling must not report wheel input")
